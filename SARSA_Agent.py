"""
Created on Tuesday Sep 15 13:33:04 CST 2020
...
class SARSA_Agent():
    ...
    def __init__(self, n_s, n_a, n_rows, n_cols, alpha, gamma, epsilon):
        ...
        # Defines the learning rate for the SARSA agent
        self.alpha = alpha
    ...
    # Updates the quality function estimate based on the SARSA algorithm
    # @param s1 - The state before an action is taken
    # @param a1 - The action taken in the first state
    # @param s2 - The state after an action is taken
    # @param a2 - The action taken in the second state
    # @param r - The reward returned traveling from s1 to s2
    ...
    # @param name - name of saved .png
    # @param path - png save filepath and name
    # @return 2d array representation of the Value function
    def visualize_V(self, V, x_label, y_label, title, x_min, y_min, x_space, y_space, name = 'sarsa_value', path=''):
        ...
        save_path = str(path) + str(name) + '.png'
    ...
    def visualize_p(self, p, x_label, y_label, title, x_min, y_min, x_space, y_space, action_title, action_min, action_space, name = 'sarsa_policy', path=''):
    ...
    def visualize_log(self, plot_trajectory, plot_total_reward, plot_discounted_reward, plot_avg_val, initial_step, final_step, path = ''):
        ...
        sub_log = {
            ...
            'curr_step': self.log['curr_step'][initial_step:final_step]
            }
        
        #plot the trajectory vs simulation step
        if plot_trajectory == True:
            plt.clf()
            title_str = "SARSA Trajectory: \u03B1 = " + str(self.alpha) + ", \u03B3 = " + str(self.gamma) + ", \u03B5 = " + str(self.epsilon)
            plt.title(title_str)
            plt.plot(sub_log['curr_step'], sub_log['s'])
            plt.plot(sub_log['curr_step'], sub_log['a'])
            plt.plot(sub_log['curr_step'], sub_log['r'])
            plt.legend(['s', 'a', 'r'])
            plt.xlabel('Learning Step')
            save_path = str(path) + 'sarsa_trajectory.png'
            plt.savefig(save_path, dpi = 500)
            plt.close()
        ...
        if plot_total_reward == True:
            plt.clf()
            title_str = "SARSA Learning Curve: \u03B1 = " + str(self.alpha) + ", \u03B3 = " + str(self.gamma) + ", \u03B5 = " + str(self.epsilon)
            plt.title(title_str)
            plt.plot([*range(len(self.log['r_tot']))], self.log['r_tot'])
            plt.xlabel('Learning Step')
            plt.ylabel('Total Reward')
            save_path = str(path) + 'sarsa_learning_curve.png'
            plt.savefig(save_path, dpi = 500)
            plt.close()
        ...
        if plot_discounted_reward == True:
            plt.clf()
            title_str = "SARSA Discounted Learning Curve: \u03B1 = " + str(self.alpha) + ", \u03B3 = " + str(self.gamma) + ", \u03B5 = " + str(self.epsilon)
            plt.title(title_str)
            plt.plot([*range(len(self.log['r_tot_discount']))], self.log['r_tot_discount'])
            plt.xlabel('Learning Step')
            plt.ylabel('Total Discounted Reward')
            save_path = str(path) + 'sarsa_discount_learning_curve.png'
            plt.savefig(save_path, dpi = 500)
            plt.close()
        ...
        if plot_avg_val == True:
            plt.clf()
            title_str = "SARSA Averag Value Curve: \u03B1 = " + str(self.alpha) + ", \u03B3 = " + str(self.gamma) + ", \u03B5 = " + str(self.epsilon)
            plt.title(title_str)
            plt.plot([*range(len(self.log['avg_value']))], self.log['avg_value'])
            plt.xlabel('Learning Step')
            plt.ylabel('Value Function  Average')
            save_path = str(path) + 'sarsa_avg_val_curve.png'
            plt.savefig(save_path, dpi = 500)
            plt.close()
